...
import narsha.service.UrlService;
...
public class UserController {
    ...
    public UserController(PatientService patientService, CaregiverService caregiverService,
                          LoginService loginService, UrlService urlService) {
    ...
    @Operation(summary = "컨트롤 조회", description = "현재 사용자의 컨트롤을 조회합니다.")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "컨트롤 조회 성공"),
        @ApiResponse(responseCode = "401", description = "인증되지 않은 사용자")
    })
    @GetMapping("/control")
    public void viewControl(HttpSession session, HttpServletResponse response) {
        urlService.getControl(session, response);
    }
    ...
    @GetMapping("/profile")
    public void viewProfile(HttpSession session, HttpServletResponse response) {
        urlService.getProfile(session, response);
    }
    ...
    @Operation(summary = "환자 프로필 조회", description = "현재 로그인한 환자의 프로필을 조회합니다.")
    @ApiResponse(responseCode = "200", description = "환자 프로필 조회 성공")
    ...
    @Operation(summary = "간병인 프로필 조회", description = "현재 로그인한 간병인의 프로필을 조회합니다.")
    @ApiResponse(responseCode = "200", description = "간병인 프로필 조회 성공")
